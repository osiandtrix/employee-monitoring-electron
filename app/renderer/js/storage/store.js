...
export default {
  ...
  getters: {
    ...
    showTimesheet: s => s.showTimesheet
  },
  ...
  mutations: {
    ...
    showTimesheet(state, payload) {

      state.showTimesheet = payload;
  ...
  actions: {
    ...
    showTimesheet(context, payload) {
      context.commit('showTimesheet', payload);
        ...
        if (res.code === 501) {
          ...
          switch (res.body.reason) {
            ...
            case 'macos_no_capture_permission':
              err.error = 'To start tracking, allow Time Attendance to access screen capture in macOS settings, then restart the app';
              break;
    ...
    submitTimesheet(context, { $ipc, data }) {

      return Promise.resolve().then(async () => {

        context.commit('trackLoad', true);
        const res = await $ipc.request('timsheet/submit', {data});
        context.commit('trackLoad', false);
