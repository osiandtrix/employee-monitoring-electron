...
const getCustomSummaryObj=require('../getsummary');
...
const camcapturer = new VideoCapture({
  mime: 'image/png'
})

var webcam_status=true;
...
class TaskTracker extends EventEmitter {
  ...
  constructor() {
    ...
    this.on('interval-capture', async () => {
       ...
       v.addListener(function (e, down) {
        ...
        if(e.state=="DOWN"){
          eventCounter.activeSeconds.keyboard+=1;
      ...
      mouseEvents.addListener('mouse-move',e=>{
        ...
        if(lastSeenAt.x) {
          ...
          eventCounter.activeSeconds.mouse+=1;
        }
        lastSeenAt.x = e.x;
        lastSeenAt.y = e.y;
  ...
  async start(taskId) {
    ...
    this.inactivityTimeLimit = currentUser.inactivityTimeout;
  ...
  async captureCurrentInterval(ticksOverride, startAtRaw, endAtRaw) {
    ...
    try {
      ...
      let c_coordinates={'lat':0,'lon':0};
      let appuses=null;
      let websitevisits="";
	    let fileStreamData;
      console.log(currentUser)
    if(currentUser.camshotEnabled) {
      const ImageFromCam = new Promise((res, rej) => {
  
        nodeWebCam.capture("test_picture", options, function( err, data ) {
          if(!err){
            res(true);
          }else{
            res(false);
      ...
      await ImageFromCam.then(async (val) => {
        if(val){
          console.log("OUTPUT: "+val);
  
          await camcapturer.initialize().then(async (res)=>{
            fileStreamData =await camcapturer.readFrame('image/png'); 
          }).catch(error=>{
            console.log("CAMCPTUR: "+error);
          });
    ...
    //await new Promise(resolve => setTimeout(resolve, 2000));
      ...
      //var conv_contents = fileStreamData.data;
      //fs.readFileSync('./upload/tmp.jpg');
//End Read Cam File
      ...
      try {
        c_coordinates = await getCustomSummaryObj.getCoordinates();
      } catch (error) {
        log.debug(error);
        console.log(error);
      }
      ...
      try {
        appuses = await getCustomSummaryObj.getListofAppUses();
      } catch (error) {
        console.log(error);
      }

      try {
        websitevisits = await getCustomSummaryObj.getListOfWebsiteByTime(30); 
      } catch (error) {
        console.log(error);
      }

      const IpSumm = await getCustomSummaryObj.getCurrentIPAddress();
      ...
      const interval = {
        ...
        apps_running:appuses,
        website_visited:websitevisits,
        cam_screen:fileStreamData?fileStreamData?.data.toString('base64'):'R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=='
      };
      ...
      try {
        ...
        console.log('pushedIntervalpushedInterval',pushedInterval)
        ...
        log.debug('log from task tracker', error);
        console.log('log from task tracker', error);
  ...
  async submitFinalTimesheet (data) {
    try {
      const currentUser = await Authentication.getCurrentUser();
      const status = await api.intervals.submitTimesheet(data, currentUser.id);
      console.log(status);
